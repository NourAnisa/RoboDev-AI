...
(() => {
  ...
  const ui = (() => {
    ...
    function build() {
      ...
      root.innerHTML = `
        <div id="zs-bar">
          ...
          <span id="zs-brand">RoboDev AI <span class="zs-free">v${EXT_VERSION}</span></span>
          ...
          <button id="zs-support" aria-label="Support RoboDev AI" title="Support RoboDev AI"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg></button>
        </div>
    ...
    try {
      chrome.storage.local.get(["zsCustomPrompt", "zs9RouterConfig"], (r) => {
        if (r && typeof r.zsCustomPrompt === "string") {
          customPrompt = r.zsCustomPrompt;
          syncMenuPrompt();
        }
        if (r && r.zs9RouterConfig && typeof r.zs9RouterConfig === "object") {
          router9Config = Object.assign(router9Config, r.zs9RouterConfig);
          sync9RouterInputs();
        }
      });
    } catch {}
    function getCustomPrompt() {
      if (router9Config && router9Config.enabled) {
        const r9Note = `[9Router AI Gateway Mode Active: Using proxy endpoint at ${router9Config.endpoint} | Target Model: ${router9Config.model}]`;
    ...
    function sync9RouterInputs() {
      if (!root) return;
      const en = root.querySelector("#zs-9r-enable");
      const ep = root.querySelector("#zs-9r-endpoint");
      const mo = root.querySelector("#zs-9r-model");
      const ky = root.querySelector("#zs-9r-key");
      if (en) en.checked = !!router9Config.enabled;
      if (ep && document.activeElement !== ep) ep.value = router9Config.endpoint || "";
      if (mo && document.activeElement !== mo) mo.value = router9Config.model || "";
      if (ky && document.activeElement !== ky) ky.value = router9Config.apiKey || "";
    }
    ...
    function buildMenu() {
      ...
      menuEl.innerHTML =
        `<div class="zs-menu-head"><span class="zs-menu-logo">RoboDev AI</span><span class="zs-menu-tag">v${EXT_VERSION}</span></div>
         ...
         <section class="zs-menu-sec">
           <div class="zs-sec-label"><span>9Router / AI Gateway</span></div>
           <div class="zs-menu-note">Hubungkan ke 9Router proxy lokal untuk auto-failover, token compression, dan multi-provider.</div>
           <div style="display:flex; align-items:center; gap:8px; margin:6px 0 8px;">
             <input type="checkbox" id="zs-9r-enable" style="cursor:pointer;" />
             <label for="zs-9r-enable" style="font-size:11.5px; cursor:pointer; color:#e5e5e8; font-weight:600;">Aktifkan 9Router Gateway</label>
           </div>
           <input id="zs-9r-endpoint" class="zs-mcp-field" placeholder="Endpoint, misal: http://127.0.0.1:20128/v1" />
           <input id="zs-9r-model" class="zs-mcp-field" placeholder="Target Model, misal: deepseek-ai/deepseek-r1" />
           <input id="zs-9r-key" type="password" class="zs-mcp-field" placeholder="API Key (opsional / jika ada)" />
           <div class="zs-set-row"><button id="zs-9r-save">Simpan 9Router</button><span id="zs-9r-status"></span></div>
         </section>
         <section class="zs-menu-sec">
      ...
      // 9Router Gateway controls
      const r9En = menuEl.querySelector("#zs-9r-enable");
      const r9Ep = menuEl.querySelector("#zs-9r-endpoint");
      const r9Mo = menuEl.querySelector("#zs-9r-model");
      const r9Ky = menuEl.querySelector("#zs-9r-key");
      const r9Save = menuEl.querySelector("#zs-9r-save");
      const r9Status = menuEl.querySelector("#zs-9r-status");
      if (r9En) r9En.checked = !!router9Config.enabled;
      if (r9Ep) r9Ep.value = router9Config.endpoint || "";
      if (r9Mo) r9Mo.value = router9Config.model || "";
      if (r9Ky) r9Ky.value = router9Config.apiKey || "";
      if (r9Save) {
        r9Save.addEventListener("click", () => {
          router9Config = {
            enabled: !!(r9En && r9En.checked),
            endpoint: (r9Ep && r9Ep.value.trim()) || "http://127.0.0.1:20128/v1",
            model: (r9Mo && r9Mo.value.trim()) || "deepseek-ai/deepseek-r1",
            apiKey: (r9Ky && r9Ky.value.trim()) || "",
          };
          try { chrome.storage.local.set({ zs9RouterConfig: router9Config }); } catch {}
          if (r9Status) {
            r9Status.textContent = "Saved ✓";
            setTimeout(() => { r9Status.textContent = ""; }, 1600);
          }
          renderBar();
        });
      }
    ...
    function buildSetup() {
      ...
      const videoBtn = VIDEO_URL
        ...
        : "";
      setupCard.innerHTML =
        `<div id="zs-setup-head"><span id="zs-setup-logo">RoboDev AI</span><span id="zs-setup-tag">Setup</span></div>` +
