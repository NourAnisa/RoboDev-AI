...
def _diagnose_roblox_error(name, text):
    """Analyze common Roblox Studio errors and append actionable advice for the AI."""
    if not text or not isinstance(text, str):
        return ""
    low = text.lower()
    advice = []
    if "attempt to index nil" in low or "is not a valid member of" in low:
        advice.append("Instance or property was nil. Use search_game_tree or inspect_instance to verify the exact instance name, hierarchy path, and capitalization.")
    if "vector3 expected" in low or "color3 expected" in low or "cframe expected" in low or "udim2 expected" in low:
        advice.append("Roblox datatype mismatch. Construct the correct datatype (e.g. Vector3.new, CFrame.new, Color3.fromRGB, UDim2.new) instead of raw strings or primitives.")
    if "edit datamodel is not available in play mode" in low:
        advice.append("Roblox Studio is currently in Play mode. Use ###LUA:Server### or ###LUA:Client### to execute live code during playtest, or use start_stop_play to stop play mode before editing permanent assets.")
    if "infinite yield possible" in low or ("waitforchild" in low and "timeout" in low):
        advice.append("WaitForChild timeout/infinite yield. Always provide a timeout (e.g. obj:WaitForChild('ChildName', 5)) and verify the child exists.")
    if "old_string" in low and ("not found" in low or "current content" in low):
        advice.append("multi_edit text mismatch. Run script_read first and copy old_string byte-for-byte including exact indentation, tabs, and line breaks.")
    if "failed to parse command code" in low or "syntax error" in low:
        advice.append("Luau syntax error. Verify matching brackets/parentheses, string quotes, and keyword endings ('end').")
    if advice:
        return "\n[DIAGNOSTIC ADVICE: " + " | ".join(advice) + "]"
    return ""
...
def safe_call(name, arguments, timeout):
    ...
    try:
        ...
        diag = _diagnose_roblox_error(name, text)
        if diag and text:
            text += diag
        return {"ok": True, "text": text, "images": result.get("images", [])}
    except TimeoutError as e:
        err = str(e)
        return {
            "ok": False,
            "error": err + "\n[DIAGNOSTIC ADVICE: Tool execution timed out. Avoid infinite loops, blocking wait() calls, or unyielding event waits inside execute_luau.]",
            "kind": "timeout",
        }
    except Exception as e:
        err = str(e)
        diag = _diagnose_roblox_error(name, err)
        return {"ok": False, "error": err + diag, "kind": type(e).__name__}
...
async def main():
    print(f"\n{C['cy']}  RoboDev AI Bridge v{BRIDGE_VERSION}{C['reset']}  {C['dim']}- Roblox Studio - ws://{HOST}:{PORT}{C['reset']}\n")
