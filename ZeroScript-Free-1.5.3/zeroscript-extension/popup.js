...
function render(s) {
  ...
  const r9El = document.getElementById("r9-status");
  if (r9El && chrome.storage && chrome.storage.local) {
    chrome.storage.local.get("zs9RouterConfig", (res) => {
      const cfg = res && res.zs9RouterConfig;
      if (cfg && cfg.enabled) {
        r9El.textContent = `9Router (${cfg.model || "Active"})`;
        r9El.style.color = "#34d399";
      } else {
        r9El.textContent = "Web Chat (Direct)";
        r9El.style.color = "#60a5fa";
      }
    });
  }
}
